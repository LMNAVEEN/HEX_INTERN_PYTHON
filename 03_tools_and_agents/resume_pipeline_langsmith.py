# Run: python 03_tools_and_agents/resume_pipeline_langsmith.py
# Tech: LangGraph + LangChain Groq + pdfplumber + LangSmith tracing
# Purpose: Same resume pipeline as resume_pipeline_groq.py but uses LangChain wrapper for LangSmith token tracking
...
# Add these to your .env (LangSmith picks them up automatically, no code needed):
#   LANGSMITH_TRACING=true
#   LANGSMITH_API_KEY=<your key>
#   LANGSMITH_PROJECT=<your project name>
#
# Because we now call the model through a LangChain Runnable (ChatGroq)
# instead of the raw Groq SDK, every traced run is typed as an "llm" run
# and LangSmith renders prompt/completion/total token counts on its own.
...
llm = ChatGroq(
    ...
    temperature=0.4,  # default; overridden per-call via .bind() in call_llm
)
...
def call_llm(prompt: str, temperature: float = 0.4) -> str:
    """
    Single LLM call via ChatGroq (a LangChain Runnable).
    Because the model is LangChain-wrapped, LangSmith automatically attaches
    usage_metadata (input_tokens / output_tokens / total_tokens) to the trace
    for every call - that's the piece the raw Groq SDK call was missing.
    """
    response = llm.bind(temperature=temperature).invoke(prompt)
...
def save_workflow_diagram(path: str = None) -> None:
    if path is None:
        path = os.path.join(BASE_DIR, "assets", "workflow_diagram.png")
    """Render the actual compiled graph (nodes + edges) to a PNG."""
    try:
        ...
        print(f"Could not generate diagram (needs internet access): {e}")
